...
import numpy as np
from drawmyLine import drawLine
...
def label2image(pred, COLORMAP=_COLORMAP):
    colormap = np.array(COLORMAP, dtype='uint8')
    X = pred.astype('int32')
    return colormap[X, :]


def trainid2id(pred, IDMAP=_IDMAP):
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            mask = label2image(seg.max(axis=0)[1].cpu().numpy(), _COLORMAP)[:, :, ::-1]
            dst, (xmin, xmax) = drawLine(mask, im0)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if xmax > int(xc) > xmin:
                            # print("dddddddddd ", xc, xmax, xmin, label)
                            # im0 = cv2.circle(im0,(xmin,100),10,color=(0,0,255))
                            # im0 = cv2.circle(im0,(xmax,100),10,color=(0,0,255))
            ...
            if opt.submit:
                ...
                result = trainid2id(seg.max(axis=0)[1].cpu().numpy(), _IDMAP)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='carvideo.mp4',
                        help='source')  # test.mp4  data/images   paper_data/seg/segimages/val

...
def train(hyp, opt, device, tb_writer=None):
    ...
    # 图尺寸相同时候用这个准确测指标（batch_size设为1可以支持图尺寸不同, 或者用下面的val mode）
        seg_valloader = SegmentationDataset.get_custom_loader(root=segval_path, batch_size=1,
                                                         ...
                                                         workers=0, pin=True)  # 验证batch_size和workers得配合, 都太大会导致子进程死亡, 单进程龟速加载数据
...
# -----------------------------------------------------------------------------------------------------------
    ...
    # BiSe用这个　两个aux
    compute_seg_loss = SegmentationLosses(nclass=3, aux=True, aux_num=2, aux_weight=0.1, ignore_index=-1, weight=None).cuda()
    # 一个aux，没有用这个
